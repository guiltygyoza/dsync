...
export const ReplicacheProvider: React.FC<ReplicacheProviderProps> = ({ children, userId: initialUserId }) => {
	...
	const rep = useMemo(() => {
		...
		if (!import.meta.env.VITE_REPLICACHE_LICENSE_KEY) {
			console.warn("VITE_REPLICACHE_LICENSE_KEY is not set in .env. Replicache will not sync.");
		}

		return getReplicacheClient(effectiveUserId);
	}, [effectiveUserId]);

	return <ReplicacheContext.Provider value={{ rep, userId: effectiveUserId }}>{children}</ReplicacheContext.Provider>;
};
