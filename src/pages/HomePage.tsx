            ...
            {category.eips.length === 0 ? (
              ...
              <ul style={{ listStyle: 'none', padding: 0 }}>
                {category.eips.map(eip => (
                  <li key={eip.id} style={{ marginBottom: '10px', padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }}>
                    <Link to={`/chamber/${eip.chamberId}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                      <h3>{eip.title} (Status: {eip.status})</h3>
                      <p>Author: {eip.author}</p>
                      <p>Chamber is: {isChamberEditable(eip.status) ? 'Editable' : 'Read-only'}</p>
                    </Link>
                  </li>
                ))}
              </ul>
