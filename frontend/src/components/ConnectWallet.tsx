    ...
    return (
      <div style={{ position: 'relative', display: 'inline-block' }} ref={dropdownRef}>
        ...
        {isDropdownOpen && (
          ...
          >
            {connector && <div style={{ marginBottom: '8px', fontSize: '0.9em', color: theme === 'dark' ? '#A0AEC0' : '#718096' }}>Connected to {connector.name}</div>}
            <button
              onClick={() => {
                disconnect();
              }}
      ...
      {isModalOpen && (
        ...
        >
          ...
          >
            ...
            {connectors.length === 0 && (
                <p style={{textAlign: 'center', color: theme === 'dark' ? '#A0AEC0' : '#718096'}}>No wallet connectors found. Please install a wallet like MetaMask.</p>
            )}
            {error && <div style={{ color: theme === 'dark' ? '#FC8181' : '#E53E3E', marginTop: '15px', textAlign: 'center' }}>{error.message}</div>}
            <button 
              onClick={() => setIsModalOpen(false)} 
              style={{ marginTop: '20px', background: themedStyles.buttonPrimaryBg, color: themedStyles.buttonPrimaryText, border: `1px solid ${themedStyles.buttonPrimaryBorder}`, padding: '8px 15px', borderRadius: '6px', cursor: 'pointer', display: 'block', marginLeft: 'auto', marginRight: 'auto' }}
            >
                Close
            </button>
          </div>
