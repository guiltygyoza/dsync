...
const walletImplementations: Wallet[] = [
  // MetaMask implementation
  {
    ...
    isInstalled: () => {
      ...
      return (
        window.ethereum !== undefined && window.ethereum.isMetaMask === true
      );
    },
    connect: async () => {
...
export const EthereumProvider: React.FC<EthereumProviderProps> = ({
  ...
  useEffect(() => {
    ...
    const checkConnection = async () => {
      ...
      if (savedWalletId) {
        const wallet = walletImplementations.find(
  ...
  const connect = async () => {
    ...
    if (wallets.length === 0) {
      alert(
        "No supported wallets detected. Please install a compatible wallet to use this feature."
      );
      return;
  ...
  const selectWallet = async (walletId: string) => {
    ...
    if (!wallet.isInstalled()) {
      alert(
        `${wallet.name} is not installed. Please install ${wallet.name} to use this feature.`
      );
      return;
    }
